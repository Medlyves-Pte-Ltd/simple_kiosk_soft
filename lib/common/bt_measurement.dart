...
class MeasurementConfiguration {
  late final List<String> measurementLabels;
  late final List<String> readingLabels;
...
class BodyTemperatureMeasurementCard extends StatefulWidget {
  const BodyTemperatureMeasurementCard({
    Key? key,
  }) : super(key: key);

  @override
  BodyTemperatureMeasurementCardState createState() =>
      BodyTemperatureMeasurementCardState();
}

class BodyTemperatureMeasurementCardState
    extends State<BodyTemperatureMeasurementCard> {
                      ...
                      child: IntrinsicWidth(
                        child: Column(
                          children: [
                            ...
                            Container(
                              color: ColorPalette.colorbodytemperature,
                              height: 1.5,
                            ),
                          ],
                        ),
  ...
  MeasurementConfiguration getConfiguration(DeviceState state) {
    String bodytemperature = userDetails['body temperature'] == ''
        ? '- - -'
        : userDetails['body temperature'];

    if (state.deviceType == DeviceType.TEMP_DEVICE) {
      if (state is DeviceDataLoading) {
        bodytemperature = AppLocalizations.of(context)!.loading;
        ...
        bodytemperature = updatedData["Bodytemperature"]!;
      }
    }
  ...
  void _onStartStopButtonPressed() {
    DeviceType deviceType = DeviceType.TEMP_DEVICE;
