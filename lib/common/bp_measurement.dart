...
class BloodPressureMeasurementCard extends StatefulWidget {
  const BloodPressureMeasurementCard({
    super.key,
  });

  @override
  BloodPressureMeasurementCardState createState() =>
      BloodPressureMeasurementCardState();
}

class BloodPressureMeasurementCardState
    extends State<BloodPressureMeasurementCard> {
                      ...
                      child: IntrinsicWidth(
                        child: Column(
                          children: [
                            ...
                            Container(
                              color: ColorPalette.colorbloodPressure,
                              height: 1.5,
                            ),
                          ],
                        ),
  ...
  MeasurementConfiguration getConfiguration(DeviceState state) {
    String bloodPressure = userDetails['bpHeartRate'] == ''
        ? '- - -'
        : '${userDetails['systolic']} / ${userDetails['diastolic']}';
    String pulse =
        userDetails['bpHeartRate'] == '' ? '- - -' : userDetails['bpHeartRate'];

    if (state.deviceType == DeviceType.BP_DEVICE) {
      if (state is DeviceDataLoading) {
        bloodPressure = pulse = AppLocalizations.of(context)!.loading;
        ...
        bloodPressure =
            "${updatedData["systolic"]} / ${updatedData["diastolic"]}";
        pulse = updatedData["bpHeartRate"]!;
      }
    }
    ...
    return MeasurementConfiguration(
      ...
      readingLabels: [bloodPressure, pulse],
    );
  }

  void _onStartStopButtonPressed() {
    DeviceType deviceType = DeviceType.BP_DEVICE;
