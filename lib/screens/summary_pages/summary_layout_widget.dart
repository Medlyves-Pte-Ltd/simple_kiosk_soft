...
class SummaryLayoutWidget extends StatelessWidget {
  ...
  final configList =
      ControlMeasurePageUtils().configList.cast<Map<String, dynamic>>();
  ...
  void init() {
    ...
    final ecgConfig = configList.firstWhere((item) => item["name"] == "ECG");
    final bcConfig =
        configList.firstWhere((item) => item["name"] == "Body Composition");
    if (bcConfig["enable"] == true) {
      _tabViewList.add(SummaryBodyComposition());
    }
    if (ecgConfig["enable"] == true) {
      _tabViewList.add(SummaryEcg());
    }
